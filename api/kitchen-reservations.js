...
module.exports=async function handler(req,res){
  ...
  try{
    if(req.method==='GET'){
      ...
      const result=await rpc('chidoliro_staff_reservations',{session_token:session,p_date:date});
      if(!result?.ok) return res.status(result?.error==='forbidden'?403:result?.error==='unauthorized'?401:400).json(result||{ok:false,error:'reservation_load_failed'});
    ...
    const result=await rpc('chidoliro_staff_update_reservation',{
      session_token:session,
      target_reservation_id:String(body.reservation_id||'').trim(),
      new_status:String(body.status||'').trim()
    });
    if(!result?.ok) return res.status(result?.error==='forbidden'?403:result?.error==='unauthorized'?401:400).json(result||{ok:false,error:'reservation_update_failed'});
    return res.status(200).json(result);
  }catch(error){
