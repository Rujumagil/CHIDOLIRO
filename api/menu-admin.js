...
function statusFor(result){return result?.error==='forbidden'?403:result?.error==='unauthorized'?401:400;}
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='GET'){
      ...
      const result=await rpc(dashboard?'chidoliro_staff_dashboard':'chidoliro_menu_admin_overview',{session_token});
      if(!result?.ok) return res.status(statusFor(result)).json(result||{ok:false,error:dashboard?'dashboard_failed':'menu_admin_load_failed'});
    ...
    if(!result?.ok) return res.status(statusFor(result)).json(result||{ok:false,error:'menu_admin_action_failed'});
    return res.status(200).json(result);
  }catch(error){
