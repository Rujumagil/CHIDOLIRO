...
module.exports = async function handler(req,res){
  ...
  try{
    const r=await fetch(`${SUPABASE_URL}/rest/v1/rpc/chidoliro_get_reservation_status`,{
    ...
    if(!r.ok) return res.status(502).json({ok:false,error:'reservation_tracking_upstream_failed'});
    if(!data?.ok) return res.status(404).json(data||{ok:false,error:'not_found'});
    return res.status(200).json(data);
  }catch(error){
    console.error('[CHIDOLIRO API] reservation tracking error',error);
    return res.status(502).json({ok:false,error:'reservation_tracking_failed'});
  }
};
