...
module.exports = async function handler(req,res){
  ...
  try{
    const r=await fetch(`${SUPABASE_URL}/rest/v1/rpc/chidoliro_resolve_table_qr`,{method:'POST',headers:{apikey:SUPABASE_ANON_KEY,Authorization:`Bearer ${SUPABASE_ANON_KEY}`,'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify({p_token:token})});
    const text=await r.text(); let data=null; try{data=JSON.parse(text||'{}')}catch(_){}
    if(!r.ok) return res.status(502).json({ok:false,error:'table_upstream_failed'});
    if(!data?.ok) return res.status(404).json(data||{ok:false,error:'table_not_found'});
    return res.status(200).json(data);
  }catch(error){console.error('[CHIDOLIRO API] table error',error);return res.status(502).json({ok:false,error:'table_failed'});}
};
