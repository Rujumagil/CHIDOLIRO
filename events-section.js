(()=> {
  ...
  .eventPoster{background:#04171b;display:grid;place-items:center;padding:16px;min-height:520px;position:relative}.eventPoster:before{content:"Cargando evento…";position:absolute;color:rgba(255,255,255,.58);font-size:.75rem;font-weight:800;letter-spacing:.08em}.eventPoster.loaded:before{display:none}.eventPoster img{display:block;width:min(100%,320px);height:auto;max-height:610px;object-fit:contain;border-radius:20px;box-shadow:0 22px 52px rgba(0,0,0,.38);opacity:0;transition:opacity .25s ease}.eventPoster.loaded img{opacity:1}
  ...
  const posterParts = Array.from({length:9}, (_,i) => `assets/evento-aniversario-b64/part-${String(i+1).padStart(2,'0')}.txt?v=1`);
  Promise.all(posterParts.map(url => fetch(url,{cache:'force-cache'}).then(r => {
    ...
    poster.onerror = () => { posterWrap.classList.add('loaded'); poster.style.display='none'; };
  }).catch(() => {
