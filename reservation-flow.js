(() => {
  ...
  function injectStyles(){
    ...
    const style=document.createElement('style');style.id='reservationFlowStyles';style.textContent=`
      ...
      .reservationSubmit{width:100%;margin-top:12px}.reservationSubmit:disabled{opacity:.55;cursor:wait}.reservationLegal{font-size:.62rem;color:var(--muted);line-height:1.45;margin:9px 3px 0;text-align:center}.eventReserveBanner{display:none;align-items:flex-start;gap:10px;background:linear-gradient(135deg,#0b438f,#1764c6);color:white;padding:12px;border-radius:15px;margin:0 0 12px}.eventReserveBanner.show{display:flex}.eventReserveBanner b{display:block;font-size:.78rem}.eventReserveBanner span{display:block;margin-top:3px;color:#dce9ff;font-size:.64rem;line-height:1.35}.eventReserveBanner .eventMark{width:34px;height:34px;flex:0 0 34px;border-radius:10px;display:grid;place-items:center;background:rgba(255,255,255,.12);font-weight:950}.eventLocked{background:#edf4ff!important;color:#17324d!important}
  ...
  function enhanceSheet(){
    ...
    grid.insertAdjacentHTML('beforebegin','<div class="eventReserveBanner" id="eventReserveBanner"><div class="eventMark">1°</div><div><b id="eventReserveTitle">1er Aniversario CHIDOLIRO</b><span id="eventReserveMeta">Domingo 18 de octubre · desde las 2 PM</span></div></div><div class="reservationIntro"><div>◷</div><div><b>Solicita tu mesa en menos de un minuto</b><span>El restaurante recibirá tu solicitud y podrás consultar aquí mismo cuando quede confirmada.</span></div></div>');
    grid.innerHTML=`
      ...
      <label class="field full"><span>Personas *</span><select id="reservationParty">${Array.from({length:12},(_,i)=>`<option value="${i+1}" ${i===1?'selected':''}>${i+1} persona${i?'s':''}</option>`).join('')}<option value="15">13–15 personas</option><option value="20">16–20 personas</option><option value="25">21–25 personas</option><option value="30">26–30 personas</option></select></label>
  ...
  function applyEventContext(ctx){
    eventContext=ctx||null;
    const banner=$('#eventReserveBanner'),head=$('#reserveSheet .sheetHead h3'),date=$('#reservationDate'),time=$('#reservationTime'),notes=$('#reservationNotes'),button=$('.reservationSubmit');
    if(!eventContext){
      banner?.classList.remove('show');
      if(head) head.textContent='Reserva tu mesa';
      if(date){date.min=today();date.max='';date.value=today();date.classList.remove('eventLocked');}
      if(time){time.min='11:00';time.max='22:00';time.value=defaultTime();}
      if(notes) notes.placeholder='Cumpleaños, silla para bebé, preferencia de mesa o alguna indicación';
      if(button) button.textContent='Solicitar reservación';
      return;
    }
    const eventDate=eventContext.date||'2026-10-18',start=(eventContext.startTime||'14:00').slice(0,5);
    if(head) head.textContent='Reserva para el aniversario';
    if(banner){banner.classList.add('show');$('#eventReserveTitle').textContent=eventContext.title||'1er Aniversario CHIDOLIRO';$('#eventReserveMeta').textContent=eventContext.meta||'Domingo 18 de octubre · desde las 2 PM';}
    if(date){date.value=eventDate;date.min=eventDate;date.max=eventDate;date.classList.add('eventLocked');}
    if(time){time.min=start;time.max='22:00';time.value=start;}
    if(notes) notes.placeholder='Preferencia de mesa o alguna indicación para el evento';
    if(button) button.textContent='Solicitar mesa para el aniversario';
  }
  function openEventReservation(ctx){
    enhanceSheet();
    applyEventContext(ctx);
    if(typeof openSheet==='function') openSheet('reserveSheet');
  }
  function clearEventContext(){if(eventContext)applyEventContext(null)}
  async function submitReservation(){
    ...
    const finalNotes=eventContext?(`[EVENTO: ${eventContext.title||'1er Aniversario CHIDOLIRO'}]`+(notes?' '+notes:'')):notes;
    ...
    try{
      const r=await fetch('/api/reservation',{method:'POST',cache:'no-store',headers:{'Content-Type':'application/json'},body:JSON.stringify({customer_name:name,customer_phone:phone,reservation_date:date,reservation_time:time,party_size:party,notes:finalNotes||null})});
      const p=await r.json().catch(()=>null);
      if(!r.ok||!p?.ok) throw new Error(p?.message||'No pudimos registrar la reservación.');
      const record={token:p.public_token,number:p.reservation_number,date:p.reservation_date,time:p.reservation_time,party:p.party_size,status:p.status,event:eventContext?{slug:eventContext.slug||'primer-aniversario-2026',title:eventContext.title||'1er Aniversario CHIDOLIRO'}:null,createdAt:Date.now()};
  ...
  function start(){injectStyles();enhanceSheet();window.reservationDemo=submitReservation;window.openEventReservation=openEventReservation;const sheet=$('#reserveSheet');sheet?.addEventListener('click',e=>{if(e.target===sheet||e.target.closest('.close'))setTimeout(clearEventContext,0)});}
